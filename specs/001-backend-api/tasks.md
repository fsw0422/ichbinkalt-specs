...
### Terraform Modules
 ...
 - [ ] T203 [P] Create infra/aws/modules/rds/ module (RDS PostgreSQL instance, DB subnet group, security group) with variables.tf, main.tf, outputs.tf
 - [ ] T204 [P] Create infra/aws/modules/dynamodb/ module (DynamoDB table with TTL configuration for auth tokens; outputs table ARN) with variables.tf, main.tf, outputs.tf
 - [ ] T205 [P] Create infra/aws/modules/secrets-manager/ module (Secrets Manager secrets for JWT signing key, DB password; outputs secret ARNs) with variables.tf, main.tf, outputs.tf
...
### Dev Environment Terragrunt Configuration
 ...
 - [ ] T208 [P] Create infra/aws/accounts/dev/account.hcl with dev AWS account ID and account name locals
 - [ ] T209 [P] Create infra/aws/accounts/dev/eu-west-1/region.hcl with region locals (aws_region = "eu-west-1")
 - [ ] T210 [P] Create infra/aws/accounts/dev/eu-west-1/vpc/terragrunt.hcl with CIDR and AZ inputs, source = modules/vpc
 - [ ] T211 [P] Create infra/aws/accounts/dev/eu-west-1/ecr/terragrunt.hcl with source = modules/ecr
 - [ ] T212 [P] Create infra/aws/accounts/dev/eu-west-1/rds/terragrunt.hcl with dependency on vpc, inputs: db.t4g.micro, single-AZ
 - [ ] T213 [P] Create infra/aws/accounts/dev/eu-west-1/dynamodb/terragrunt.hcl with table name and TTL attribute inputs
 - [ ] T214 [P] Create infra/aws/accounts/dev/eu-west-1/secrets-manager/terragrunt.hcl with secret name inputs (JWT key, DB password)
 - [ ] T215 Create infra/aws/accounts/dev/eu-west-1/ecs/terragrunt.hcl with dependencies on vpc, ecr, rds, dynamodb, secrets-manager, and environment-specific inputs (depends on T210-T214)

**Checkpoint**: Dev environment infrastructure definition complete. `terragrunt run-all apply` from accounts/dev/eu-west-1/ provisions the full stack in dependency order (vpc → ecr/rds/dynamodb/secrets-manager → ecs).
...
### Shared User Domain Entities
 ...
 - [ ] T017 [P] [Shared] Create src/core/entities/ directory and UserEntity.ts with userId, displayName, email, createdAt, updatedAt, lastActiveAt
...
### Shared User Ports and Services
 ...
 - [ ] T073 [P] [Shared] Create src/core/ports/secondary/ directory and IUserRepository.ts (create, findById, update, findByEmail)
 - [ ] T074 [P] [Shared] Create src/core/ports/primary/ directory and IUserService.ts
 - [ ] T075 [Shared] Create src/adapters/secondary/typeorm/ directory and implement UserRepository.ts with mapping between domain and ORM entities, including eager loading of learningProfile, usageStats, preferences (depends on T017, T067-T069, T073)
 - [ ] T076 [Shared] Create src/core/services/ directory and implement UserService.ts: create user with default sub-records (used by AuthService), get full user profile, update displayName, update learning profile, update preferences (depends on T073, T074)
...
### Domain Entities for User Story 1
 ...
 - [ ] T084 [P] [US1] Create MagicLinkTokenEntity.ts in src/core/entities/ (tokenId, email, tokenHash, expiresAt, usedAt, createdAt)
...
### Domain DTOs for User Story 1
 ...
 - [ ] T089 [P] [US1] Create AuthResponseDto.ts in src/core/dtos/ (accessToken, refreshToken, userId, passkeyRegistrationRequired)
...
### Port Interfaces for User Story 1
 ...
 - [ ] T092 [P] [US1] Create IUserPasskeyRepository.ts in src/core/ports/secondary/ (create, findByUserId, findByCredentialId, updateCounter)
 - [ ] T093 [P] [US1] Create IRefreshTokenRepository.ts in src/core/ports/secondary/ (create, findByTokenHash, revoke, revokeAllForUser)
 - [ ] T094 [P] [US1] Create IEmailService.ts in src/core/ports/secondary/ (sendMagicLinkEmail)
 - [ ] T095 [P] [US1] Create IAuthService.ts in src/core/ports/primary/ 

### ORM Entities for User Story 1

 - [ ] T096 [P] [US1] Create src/adapters/secondary/typeorm/entities/ directory and MagicLinkTokenOrmEntity.ts
 - [ ] T097 [P] [US1] Create UserPasskeyOrmEntity.ts in src/adapters/secondary/typeorm/entities/
 - [ ] T098 [P] [US1] Create RefreshTokenOrmEntity.ts in src/adapters/secondary/typeorm/entities/

### Database Migrations for User Story 1

 - [ ] T099 [P] [US1] Create db/changelog/ directory and Liquibase migration for magic_link_tokens table
 - [ ] T100 [P] [US1] Create Liquibase migration for user_passkeys table in db/changelog/
 - [ ] T101 [P] [US1] Create Liquibase migration for refresh_tokens table in db/changelog/

### Persistence Adapters for User Story 1

 - [ ] T102 [US1] Implement MagicLinkTokenRepository.ts in src/adapters/secondary/typeorm/ (depends on T091, T096)
 - [ ] T103 [US1] Implement UserPasskeyRepository.ts in src/adapters/secondary/typeorm/ (depends on T092, T097)
 - [ ] T104 [US1] Implement RefreshTokenRepository.ts in src/adapters/secondary/typeorm/ (depends on T093, T098)

### Email Adapter for User Story 1

 - [ ] T105 [US1] Create src/adapters/secondary/email/ directory and implement StubEmailService.ts behind IEmailService port (depends on T094). Logs magic link to console for local development.

### Services for User Story 1

 - [ ] T106 [US1] Implement AuthService in src/core/services/AuthService.ts: magic link request/verify (calls UserService.create for new users), JWT issuance (access+refresh), token refresh, logout/revocation, rate limiting (5/email/hour) (depends on T091-T095, T073, T076)
 - [ ] T107 [US1] Implement PasskeyService in src/core/services/PasskeyService.ts: registration options, register credential, authentication options, authenticate. Use @simplewebauthn/server (depends on T092, T095)
...
### Primary Adapters (REST Controllers) for User Story 1
 ...
 - [ ] T110 [US1] Create PasskeyController in src/adapters/primary/rest/PasskeyController.ts with endpoints: POST registration-options, POST register, POST authentication-options, POST authenticate (depends on T107)
...
### Integration & Validation for User Story 1
 ...
 - [ ] T115 [US1] Validate magic link verify creates new User with defaults when email is not found (depends on T109)
 - [ ] T116 [US1] Validate 429 rate limiting on magic link requests (depends on T109)
 - [ ] T117 [US1] Validate 401 on expired/used magic link token (depends on T109)
...
**Checkpoint**: User Story 1 complete. Users can sign up via magic link, register passkeys, and obtain JWTs for protected APIs.

---

## Phase 5: User Story 2 - User Profile Management API (Priority: P1)

**Goal**: Enable authenticated users to retrieve their full profile and update learning preferences and app preferences via REST APIs. User creation is handled by magic link verification (User Story 1).
...
### Persistence Adapter for User Story 3
 ...
 - [ ] T029 [US3] Implement ConversationRepository.ts in src/adapters/secondary/typeorm/ (depends on T019, T025)
